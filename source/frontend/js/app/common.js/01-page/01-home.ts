...
namespace AvsHome {
  ...
  export interface IGetVerificationPayloadAndUrlResponse
  ...
  export interface IValidateVerificationPayloadResponse
    extends ISuccessResponse {
    sessionId: number;
  }
  ...
  export namespace Action {
    let exampleImplementationStartRedirectButton = $(
      "#exampleImplementationStartRedirectButton"
    );
    let exampleImplementationStartJsButton = $(
      "#exampleImplementationStartJsButton"
    );
    let exampleImplementationIframeJsButton = $(
      "#exampleImplementationIframeJsButton"
    );
    ...
    let colorConfigButtonBackgroundInput = $(
      "#colorConfigButtonBackgroundInput"
    );
    let colorConfigButtonForegroundInput = $(
      "#colorConfigButtonForegroundInput"
    );
    let colorConfigButtonForegroundCTAInput = $(
      "#colorConfigButtonForegroundCTAInput"
    );

    let accessInformationCallbackUrlInput = $(
      "#accessInformationCallbackUrlInput"
    );
      ...
      exampleImplementationStartRedirectButton.on("click", (e) => {
        ...
        var postData = {
          ...
          colorConfigButtonForegroundInput: colorConfigButtonForegroundInput
            ...
            .toString(),
          colorConfigButtonForegroundCTAInput:
            colorConfigButtonForegroundCTAInput.val().toString(),
    ...
    function iframeAvsHandler(iframeUrl: string) {
      if (avsInstance === null) {
        AvsFactoryIframeSdk.V1.Config.create({
          verificationType:
            AvsFactoryIframeSdk.V1.Config.VERIFICATION_TYPE_DETECTION,
      ...
      avsInstance.onMessage = (eventMessage: IAvsInstanceEvent) => {
        ...
        switch (eventMessage.name) {
          case AvsFactoryIframeSdk.V1.Config.EVENT_ON_CHECK_IFRAME_LOADED:
            ...
            avsInstance.emit(
              AvsFactoryIframeSdk.V1.Config.EVENT_STATUS_REQUEST
            );
          ...
          case AvsFactoryIframeSdk.V1.Config
            ...
            appendToLog(
              "Verification fatal error: " + JSON.stringify(eventMessage.data)
            );
  ...
  export namespace Ajax {
    export function getVerificationPayloadAndUrl(
      postData: IGetVerificationPayloadAndUrlData
    ) {
    ...
    export function validateVerificationPayload(
      postData: IValidateVerificationPayloadData
    ) {
      return Core.Ajax.post(
