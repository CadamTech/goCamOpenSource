    ...
    export namespace Library {
      export namespace Ml {
        export class FaceApi extends Avs.Plugin.Handler {
          ...
          static readonly DETECTOR_TYPE_TINY_FACE_DETECTOR =
            "tiny_face_detector";
          ...
          constructor(
            ...
            if (typeof faceapi === "undefined") {
              this.debug.error(
                25011,
                "Required face detection library not loaded."
              );
            ...
            this.videoElement = <HTMLVideoElement>(
              $(this.config.videoElementSelector).get(0)
            );
            this.canvasOverlayElement = <HTMLCanvasElement>(
              $(this.config.canvasOverlayElementSelector).get(0)
            );
                  ...
                  .then(
                    (result: any) => {
                      ...
                      this.detectorOptions =
                        new this.faceApiClass.SsdMobilenetv1Options({
                ...
                this.faceApiClass.nets.tinyFaceDetector
                  ...
                  .then(
                    (result: any) => {
                      ...
                      this.detectorOptions =
                        new this.faceApiClass.TinyFaceDetectorOptions({
                          inputSize: FaceApi.TINY_FACE_DETECTOR_INPUT_SIZE,
                          scoreThreshold:
                            FaceApi.TINY_FACE_DETECTOR_SCORE_THRESHOLD,
                        });
          ...
          public detectFaceAge(cb: Function) {
            ...
            if (!this.ageGenderLoaded) {
              this.debug.error(
                25015,
                "Face detection age and gender model must be loaded"
              );
              cb(null);
              return;
            }
              ...
              .then(
                (result: any) => {
                  ...
                  if (typeof result !== "undefined" && result.detection) {
                    ...
                    const averageAge = parseInt(
                      this.interpolateAgePredictions(age).toFixed()
                    );
          ...
          public detectFaceFromCustomElement(
            ...
            if (!this.recognitionModelLoaded) {
              this.debug.error(
                25049,
                "Face detection recognition model must be loaded"
              );
              cb(null);
              return;
            }
            ...
            this.faceApiClass
              ...
              .then(
                (result: any) => {
                  // face was detected
                  if (typeof result !== "undefined") {
                    this.drawFaceAgeResultsToCustomElement(
                      result,
                      1,
                      1,
                      $(".documentProcessingCanvasOverlay").get(0),
                      customElement
                    );
          ...
          public extractFaceImageFromCustomElement(
            detectedFace: any,
            customElement: HTMLElement,
            cb: Function
          ) {
          ...
          public detectFaceExpression(cb: Function) {
            ...
            if (!this.faceExpressionModelLoaded) {
              this.debug.error(
                25060,
                "Face api expression model must be loaded"
              );
              cb(null);
              return;
            }
                  ...
                  if (
                    ...
                    for (let i in result.expressions) {
                      if (result.expressions.hasOwnProperty(i)) {
                        ...
                        if (
                          expressionValue >= FaceApi.EXPRESSION_MIN_CONFIDENCE
                        ) {
          ...
          public compareFaceDescriptors(
            face1Descriptors: any,
            face2Descriptors: any
          ) {
            const originalDistance = this.faceApiClass.euclideanDistance(
              face1Descriptors,
              face2Descriptors
            );
          ...
          public drawFaceAgeResults(
            result: any,
            age: number,
            averageAge: number
          ) {
            const dims = this.faceApiClass.matchDimensions(
              this.canvasOverlayElement,
              this.videoElement,
              true
            );
            const resizedResult = this.faceApiClass.resizeResults(result, dims);

            this.faceApiClass.draw.drawDetections(
              this.canvasOverlayElement,
              resizedResult
            );
          ...
          public drawFaceAgeResultsToCustomElement(
            result: any,
            age: number,
            averageAge: number,
            canvasOverlay: HTMLElement,
            customElement: HTMLElement
          ) {
            const dims = this.faceApiClass.matchDimensions(
              canvasOverlay,
          ...
          public interpolateAgePredictions(age: number) {
            ...
            return (
              this.predictAgeList.reduce(
                (total: number, a: number) => total + a
              ) / this.predictAgeList.length
            );
          }

          public extractFaceCoordinates(
            faceDetection: IFaceDetectionCoordinates
          ): IFaceCoordinates {
            return {
