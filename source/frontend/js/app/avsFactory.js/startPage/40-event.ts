  ...
  export namespace StartPage {
    export class Event {
      public static init(): void {
        ...
        instance.event.on(
          Avs.Helper.FilePreloader.ON_PRELOAD_EVENT +
            "." +
            Config.FACE_API_PRELOADER_NAME,
          (event: Avs.Event.Listener, data: Avs.Helper.IFileListInfo) => {
            instance.ui.FaceApiPreloaderPercentArea.setContent(
              data.total.progress + "%"
            );

            if (
              data.total.progress == 100 &&
            ...
            if (
              data.total.status === Avs.Helper.FilePreloader.FILE_STATUS_ERROR
            ) {
              Method.renderError(
                25032,
                "Preloading verification resources failed"
              );
              return;
            }
          }
        ...
        instance.event.on(
          ...
          (event: Avs.Event.Listener, data: Avs.Helper.IFileListInfo) => {
            instance.ui.TesseractPreloaderPercentArea.setContent(
              data.total.progress + "%"
            );
        ...
        instance.event.on(
          ...
          (event: Avs.Event.Listener, data: Avs.IApiResponseData) => {
            if (typeof data.isValidated !== "undefined" && data.isValidated) {
              ...
              instance.entity.VerificationStepGlobal.deviceLocationVerification =
